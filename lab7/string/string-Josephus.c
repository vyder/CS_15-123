...
void loadList( CDLL *list, char *infileName)
{
	...
	while( fgetline_123( &name, infile ) )
		insertAtTail( /* Y O U R   C O D E   H E R E */  );
...
int main(int argc, char *argv[])
{
	...
	initList(	/* Y O U R   C O D E   H E R E */);
	loadList(	/* Y O U R   C O D E   H E R E */);
	...
	do /* prompt user for a member to start the counting with */
	{
		...
		currNode = searchList( 	/* Y O U R   C O D E   H E R E */);
		free( name );
	} while (!currNode);
	...
	while (list.head) /* while list has any members left */
	{
		...
		currNode = deleteNode( /* Y O U R   C O D E   H E R E */  );
		if (!currNode)
		{
